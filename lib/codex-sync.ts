...
const WINDOWS_RENAME_RETRY_ATTEMPTS = 8;
	...
	for (let attempt = 0; attempt < WINDOWS_RENAME_RETRY_ATTEMPTS; attempt += 1) {
		try {
			...
			if (isWindowsLockError(error)) {
				lastError = error;
				const jitterMs = Math.floor(Math.random() * WINDOWS_RENAME_RETRY_BASE_DELAY_MS);
				await new Promise((resolve) =>
					setTimeout(
						resolve,
						WINDOWS_RENAME_RETRY_BASE_DELAY_MS * 2 ** attempt + jitterMs,
					),
				);
				continue;
			}
